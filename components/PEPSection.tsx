...
function bestLabel(labels: string[]) {
  ...
  for (const label of labels) {
    if (isTitlecase.exec(label) !== null)
      return label;
  }
  return labels[0];
}
...
export function PEPSubsection({ subsectionDefinition, positions, countryCode, hardLimit }: PEPSubsectionProps) {
  ...
  return (
    ...
    <tbody key={subsectionDefinition.name}>
      <tr>
        ...
        <th className="numeric d-none d-md-table-cell text-nowrap">
          ...
          <HelpLink href="#explain-status-unclear" tooltipId="status-unclear">
            When we can not determine whether
            a person currently holds a position from our data source(s) with sufficient confidence,
            we indicate its status as unclear.
          </HelpLink>
        </th>
      </tr>
      ...
      {positions.length > limit && limit < hardLimit &&
        <tr>
          <td colSpan={4}>
            <button className="btn btn-link" onClick={() => setLimit(limit + PAGE_LIMIT)}>
              Show more
            </button>
            <Spacer />
            <button className="btn btn-link" onClick={() => setLimit(hardLimit)}>
              Show all
            </button>
          </td>
        </tr>
