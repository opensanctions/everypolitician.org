...
async function createCacheConfig() {
  ...
  const lruCache = createLruHandler(lruSettings);
  ...
  const config = {
    handlers: [
      createCompositeHandler({
        ...
        setStrategy: (ctx) => (ctx?.tags.includes("memory") ? 0 : 1),
      }),
    ],
  };
...
CacheHandler.onCreation(() => {
  ...
  if (process.env.NODE_ENV === "development") {
    const config = { handlers: [createLruHandler(lruSettings)] };
    if (isSingleConnectionModeEnabled) {
      global.cacheHandlerConfig = config;
    }
    return config;
